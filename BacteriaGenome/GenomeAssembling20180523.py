...
TRIMMOMATIC = '/mnt/d/linux/P/Trimmomatic-0.38/trimmomatic-0.38.jar'
SOAPDENOVO = '/mnt/d/linux/P/SOAPdenovo2-master/SOAPdenovo-127mer'
GAPCLOSER = '/mnt/d/linux/P/SOAPdenovo2-master/GapCloser'
ILLUMINACLIP = '/mnt/d/linux/P/Trimmomatic-0.38/adapters/TruSeq3-PE.fa'
KMERFREQ = '/mnt/d/linux/P/SOAPec_bin_v2.03/bin/KmerFreq_AR'
CORRECTOR = '/mnt/d/linux/P/SOAPec_bin_v2.03/bin/Corrector_AR'
FASTQC = '/mnt/d/linux/P/FastQC/fastqc'
...
def geneAssembling(folder, file_fq1, file_fq2, outprefix='', kmers=[35, 47, 59, 71, 83, 95, 107, 119]):
    ...
    # check quality
    command_fastqc = '{FASTQC} ../{file_fq1} ../{file_fq2} -t 8'.format(FASTQC=FASTQC, file_fq1=file_fq1,file_fq2=file_fq2)
    ...
    command_kmer = '{KMERFREQ}  -k 17 -t 4 -q 33 {file_correct}'.format(KMERFREQ=KMERFREQ, file_correct=file_correct)
    subprocess.run(command_kmer,shell=True)
    log_info.write(TIME()+ '  Finished counting kmers, kmer=17\n\n')
    
    ## Correct reads
    command_correct = '{CORRECTOR}  -k 17 -t 4 -Q 33 -o 3 output.freq.cz output.freq.cz.len {file_correct} '.format(CORRECTOR=CORRECTOR, file_correct=file_correct)
...
#        f.write('q={file_fq1}.single.cor.single.fq\n'.format(file_fq1=file_fq1))
    ...
    command_soaps = []
    for kmer in kmers:
        command_soap = '{SOAPDENOVO} all -s {file_config} -K {kmer} -R -o {outprefix}.kmer{kmer} -d 1 '.format(SOAPDENOVO=SOAPDENOVO, folder=folder, file_config=file_config, kmer=kmer, outprefix=outprefix)
    ...
    for _n in range(len(lis_options)):
        command_soap = '{SOAPDENOVO} all -s {file_config} -K {bestkmer} -o {outprefix}.bestkmer{_n} {option}'.format(SOAPDENOVO=SOAPDENOVO, folder=folder, file_config=file_config, bestkmer=bestkmer, outprefix=outprefix, _n=_n, option = lis_options[_n])
        subprocess.run(command_soap, shell=True)
        log_info.write(TIME()+ '  Finished soapdenovo for setting: {option}\n'.format(option=lis_options[_n]))
    
    for _n in range(len(lis_options)):
        command_gap = '{GAPCLOSER} -a {outprefix}.bestkmer{_n}.scafSeq -b {file_config} -o {outprefix}.bestkmer{_n}.scafSeqFilled -l 155 -t 8'.format(GAPCLOSER=GAPCLOSER, folder=folder, file_config=file_config, outprefix=outprefix, _n=_n)
